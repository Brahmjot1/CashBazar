...
public class MainActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    ArrayList<String> slideMenuListIcon=new ArrayList<>();
    ...
    private void fetchData() {
        ...
        call.enqueue(new Callback<ResponseBody>() {
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                ...
                try {
                    ...
                    if (response.isSuccessful()) {
                        ...
                        slideMenuList(slidemenulistarray);
            ...
            {
                final int scrollDuration = 3000; // Adjust this value to change the scroll duration
                ...
                autoScrollRunnable = new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (nextPosition >= bannerArrayList.size()) {
                            nextPosition = 0; // Start from the beginning if reached the end
                        }

                        recyclerView.smoothScrollToPosition(nextPosition);
                        autoScrollHandler.postDelayed(this, scrollDuration);
                    }
                };

                autoScrollHandler.postDelayed(autoScrollRunnable, scrollDuration);
            }
    ...
    private void slideMenuList(@NonNull JSONArray slidemenulistarray) {
        try
        {
            Menu menu=nav.getMenu();

            for (int i = 0; i<slidemenulistarray.length(); i++) {
                JSONObject jb=new JSONObject(slidemenulistarray.get(i).toString());
                String icone = jb.getString("icone");
                slideMenuListIcon.add(new String(icone));
            }

            System.out.println(slideMenuListIcon);

            for (int i = 0; i < slideMenuListIcon.size(); i++) {
                String iconName = String.valueOf(slideMenuListIcon.get(i));
                int iconResId = getResources().getIdentifier(iconName, "drawable", getPackageName());
                MenuItem menuItem = menu.add(Menu.NONE, i, Menu.NONE, "Item " + (i + 1));
